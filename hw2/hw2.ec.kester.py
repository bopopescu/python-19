...
# September 12, 2014
# hw2.ec.kester.py
# Password strength checker with extra credit
#---------------------------------------------------------
...
def search_recursive(password_list, pw, low, high):
    if (high < low):
        return -1
    else:
        mid = low + ((high - low) /2)
...
while var == 1:
    ...
    #User input getpass() only functions with tty, non-terminal redirect
    if sys.stdin.isatty():
